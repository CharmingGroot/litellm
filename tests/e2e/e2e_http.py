...
class MultipartFile:
    """The file part of a multipart/form-data upload. Carries either raw bytes or a
    path to stream from (not a pydantic model, since the content goes straight to the
    encoder, never serialized to JSON). Use ``path`` for large uploads so the file is
    streamed off disk instead of held in memory."""
...
def upload[R: BaseModel](
    ...
    *,
    ...
    handle = open(file.path, "rb") if file.path is not None else None
    payload = handle if handle is not None else file.content
    try:
        resp = requests.post(
            str(url),
            headers=_headers(headers),
            data=form.model_dump(by_alias=True, exclude_none=True),
            files={file.field_name: (file.filename, payload, file.content_type)},
            timeout=timeout,
        )
    except requests.RequestException as exc:
        return NetworkError(message=str(exc))
