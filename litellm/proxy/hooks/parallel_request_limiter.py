...
class _PROXY_MaxParallelRequestsHandler(CustomLogger):
    ...
    async def async_pre_call_hook(
        ...
        tpm_limit = throttled_limit(
            getattr(user_api_key_dict, "tpm_limit", sys.maxsize), throttle_pct
        )
        if tpm_limit is None:
            tpm_limit = sys.maxsize
        rpm_limit = throttled_limit(
            getattr(user_api_key_dict, "rpm_limit", sys.maxsize), throttle_pct
        )
